...
def load_model_from_checkpoint(config, ckpt, device, verbose=False):
...
def init_models(device, ckpt, half_precision=False):
    config_path = os.path.join(os.path.dirname(__file__), '../configs/sd-objaverse-finetune-c_concat-256.yaml')
    ...
    if half_precision:
        models['turncam'] = torch.compile(load_model_from_checkpoint(config, ckpt, device=device)).half()
    else:
        models['turncam'] = torch.compile(load_model_from_checkpoint(config, ckpt, device=device))
...
def predict_stage1_gradio(model, raw_im, save_path="", adjust_set=[], device="cuda", ddim_steps=75, scale=3.0):
    ...
    for stage1_idx in range(len(delta_x_1_8)):
        ...
        if save_path:
            out_image.saveos.path.join(save_path, '%d.png'%(stage1_idx))
        sample_idx += 1
